...
class Message
{
    ...
    public:
        ...
        QString &content() ; //Permet de récuperer le contenu du message
        void setContent(const QString &newContent); //Permet de modifier le contenu du message

        QHostAddress senderIP() const; //Permet de récuperer l'adresse Ip du destinateur
        void setSenderIP(QHostAddress newSenderIP); //Permet de modifier l'adresse Ip du destinateur

        const QDateTime &date_time() const;
        void setDate_time(const QDateTime &newDate_time);
...
private:
         ...
         * qui sont des fonctions amies pour l'envoi et la reception de paquets message*/
